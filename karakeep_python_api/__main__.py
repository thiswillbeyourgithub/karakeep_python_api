        ...
        def command_func(ctx, **kwargs):
            ...
            try:
                ...
                for p_name, p_obj in sig_params.items():
                    ...
                    # Check if this parameter corresponds to the '--data' option added later
                    # This relies on the convention added in the parameter loop below
                    if (
                        is_body_type and p_name + "_is_body" in kwargs
                    ):  # Check for marker
                ...
                for cli_name, value in kwargs.items():
                    # Skip the marker argument
                    if cli_name.endswith("_is_body"):
                        continue

                    py_name = cli_name.replace(
                        "-", "_"
                    ...
                    elif py_name in sig_params:
                        # Handle boolean flags (value is True/False from Click)
                        param_annotation = sig_params[py_name].annotation
                        is_bool_param = param_annotation is bool or (

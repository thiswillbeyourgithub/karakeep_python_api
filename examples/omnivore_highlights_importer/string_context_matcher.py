...
class MatchResult:
    ...
    matches: List[str]
    ratio: float
    distance: float # Levenshtein distance is usually int, but normalized_distance is float
...
def match_highlight_to_corpus(
    ...
    if indexes:
        ...
        if not batches: # No suitable batches found
            ...
            for current_region_idx_in_batches in max_rat_idx:
                ...
                end_slice = current_region_idx_in_batches + 1 # Slicing is exclusive at end
                ...
                try:
                    ...
                    if batches and current_region_idx_in_batches < len(batches):
                         try:
                            iidx = mini_corp.index(batches[current_region_idx_in_batches])
                         except ValueError:
                             continue # Skip this max_rat_idx if problematic
                    else:
                        continue # Skip this max_rat_idx if problematic

                area = mini_corp[iidx : iidx + 3 * lq] # Define search area in mini_corp
                if not area.strip():
                    continue
                ...
                if current_batch_max_r >= best_ratio and current_batch_min_d <= best_dist:
                    # Find all strings in batches2 that yield current_batch_max_r
                    indices_for_current_max_r = [i for i, r_val in enumerate(ratios2) if r_val == current_batch_max_r]
                    
                    if not indices_for_current_max_r: # Should not happen if current_batch_max_r is from ratios2
                        continue
            ...
            if best_matches:
                ...
                return MatchResult(matches=best_matches, ratio=best_ratio, distance=best_dist, quick_match_used=True)
    ...
    if query_len == 0: return MatchResult(matches=[], ratio=0.0, distance=1.0, quick_match_used=False) # Or handle as error
    if corpus_len == 0: return MatchResult(matches=[], ratio=0.0, distance=1.0, quick_match_used=False)
    ...
    right_boundary = min(corpus_len, (closest_match_corpus_start_idx + query_len -1) + query_len_by_2 + 2)
    ...
    ngram_lens_thorough = [
        l for l in range(narrowed_corpus_len, query_len_by_2 - 1, -query_len_by_step_factor) if l > 0
    ]
    if not ngram_lens_thorough: # If narrowed_corpus_len is too small or other edge cases
        ngram_lens_thorough.append(min(query_len, narrowed_corpus_len)) # Ensure at least one sensible length
    ...
    for ngram_len in ngram_lens_thorough:
        if ngram_len > narrowed_corpus_len : continue # Should not happen if ngram_lens_thorough is generated correctly
        current_set_compare = [
    ...
    if not narrowed_corpus_ngrams_thorough_sets:
        ...
        if dist_val <= min_dist_val : # Using min_dist_val from initial pass
             return MatchResult(matches=[narrowed_corpus_original_case], ratio=ratio_val, distance=dist_val, quick_match_used=False)
             ...
             best_ngram_str_initial_pass_original_case = corpus[best_ngram_from_initial_pass_idx * query_len_by_2 : best_ngram_from_initial_pass_idx * query_len_by_2 + query_len]
             ratio_for_initial_best = lev_ratio(best_ngram_str_initial_pass_original_case.casefold() if not case_sensitive else best_ngram_str_initial_pass_original_case, query_to_compare)
             return MatchResult(matches=[best_ngram_str_initial_pass_original_case], ratio=ratio_for_initial_best, distance=min_dist_val, quick_match_used=False)
    ...
    for i_set, ngram_set_original_case in enumerate(narrowed_corpus_ngrams_original_case_sets):
    ...
    if not final_best_matches:
        ...
        original_string_match = corpus[idx * query_len_by_2 : idx * query_len_by_2 + query_len]
    ...
    all_ratios = [lev_ratio((bm.casefold() if not case_sensitive else bm), query_to_compare) for bm in final_best_matches]
    ...
    return MatchResult(matches=final_best_matches, ratio=best_ratio_val, distance=min_dist_val, quick_match_used=False) # False: used long way

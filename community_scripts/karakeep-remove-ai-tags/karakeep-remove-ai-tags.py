...
def main(dry_run: bool = False):
    ...
    print(f"Found {len(ai_only_tags)} tags that are attached by AI and have no human attachments")
    
    # List the tags that will be removed
    if ai_only_tags:
        print("\nTags that will be removed:")
        for tag in ai_only_tags:
            print(f"- {tag.name} (ID: {tag.id}, AI attachments: {tag.numBookmarksByAttachedType.ai})")
    else:
